...
export default function SignupPage() {
  ...
  async function handleSignup() {
    ...
    try {
      ...
      await apiFetch("/auth/register", {
        method: "POST",
        body: JSON.stringify({ first_name: firstName.trim(), last_name: lastName.trim(), email, password }),
      });
  ...
  const canSubmit = !loading && !!firstName.trim() && !!lastName.trim() && !!email.trim() && !!password && !!confirmPassword && password === confirmPassword;
  ...
  return (
    <div style={{
      ...
      <div style={{ width: "100%", maxWidth: 440, margin: "0 16px", position: "relative", zIndex: 1 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 14, color: "var(--accent-light)", opacity: 0.75, marginTop: 6 }}>
            Start collaborating on Stay on Track
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: "grid", gap: 14 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <InputField label="First name" placeholder="Jane" value={firstName} onChange={setFirstName}
                onKeyDown={(e) => e.key === "Enter" && handleSignup()} />
              <InputField label="Last name" placeholder="Smith" value={lastName} onChange={setLastName}
                onKeyDown={(e) => e.key === "Enter" && handleSignup()} />
            </div>
            <InputField label="Email" placeholder="jane@example.com" value={email} onChange={setEmail}
              onKeyDown={(e) => e.key === "Enter" && handleSignup()} />
            <InputField label="Password" placeholder="Min. 8 characters" value={password} type="password"
              onChange={setPassword} onKeyDown={(e) => e.key === "Enter" && handleSignup()} />
            <div>
              <InputField label="Confirm password" placeholder="Re-enter your password" value={confirmPassword} type="password"
                onChange={setConfirmPassword} onKeyDown={(e) => e.key === "Enter" && handleSignup()} />
              {confirmPassword && password !== confirmPassword && (
                <div style={{ fontSize: 12, color: "#ef4444", marginTop: 5, paddingLeft: 2 }}>
